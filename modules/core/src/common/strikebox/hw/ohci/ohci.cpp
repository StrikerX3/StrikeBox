...
OHCI::OHCI(virt86::VirtualProcessor& vp, int Irq, USBPCIDevice* UsbObj)
    : m_vp(vp)
{
    int offset = 0;
    USBPortOps* ops;
...
{
	if (Paddr != 0) {
        m_vp.LMemRead(Paddr, sizeof(*Ed), Ed);
		return false;
	}
	return true; // error
}
...
bool OHCI::OHCI_WriteED(uint32_t Paddr, OHCI_ED* Ed)
{
	if (Paddr != 0) {
        ...
        m_vp.LMemWrite(Paddr, 4, Ed + OffsetOfHeadP);
		return false;
	}
	return true; // error
}
...
{
	if (Paddr != 0) {
        m_vp.LMemRead(Paddr, sizeof(*Td), Td);
		return false;
	}
	return true; // error
}
...
{
	if (Paddr != 0) {
        m_vp.LMemWrite(Paddr, sizeof(*Td), Td);
		return false;
	}
	return true; // error
}
...
bool OHCI::OHCI_CopyTD(OHCI_TD* Td, uint8_t* Buffer, int Length, bool bIsWrite)
{
	...
	// From the standard: "If during the data transfer the buffer address contained in the HC's working copy of
