 ...
 * Constructor
 */
GdbServer::GdbServer(VirtualProcessor& vp, const char *bind_host, int bind_port)
: m_vp(vp), m_bind_host(bind_host), m_bind_port(bind_port)
{
}
...
int GdbServer::Debug(int signal)
{
    ...
    // Load Registers
    {
        ...
        RegValue vals[ARRAY_SIZE(regs)];

        m_vp.RegRead(regs, vals, ARRAY_SIZE(regs));

        m_dbg_state.registers[DBG_CPU_I386_REG_EAX] = vals[0].u32;
        m_dbg_state.registers[DBG_CPU_I386_REG_ECX] = vals[1].u32;
        m_dbg_state.registers[DBG_CPU_I386_REG_EDX] = vals[2].u32;
        m_dbg_state.registers[DBG_CPU_I386_REG_EBX] = vals[3].u32;

        m_dbg_state.registers[DBG_CPU_I386_REG_ESP] = vals[4].u32;
        m_dbg_state.registers[DBG_CPU_I386_REG_EBP] = vals[5].u32;
        m_dbg_state.registers[DBG_CPU_I386_REG_ESI] = vals[6].u32;
        m_dbg_state.registers[DBG_CPU_I386_REG_EDI] = vals[7].u32;

        m_dbg_state.registers[DBG_CPU_I386_REG_PC] = vals[8].u32;
        m_dbg_state.registers[DBG_CPU_I386_REG_PS] = vals[9].u32;

        m_dbg_state.registers[DBG_CPU_I386_REG_CS] = vals[10].segment.selector;
        m_dbg_state.registers[DBG_CPU_I386_REG_SS] = vals[11].segment.selector;
        m_dbg_state.registers[DBG_CPU_I386_REG_DS] = vals[12].segment.selector;
        m_dbg_state.registers[DBG_CPU_I386_REG_ES] = vals[13].segment.selector;
        m_dbg_state.registers[DBG_CPU_I386_REG_FS] = vals[14].segment.selector;
        m_dbg_state.registers[DBG_CPU_I386_REG_GS] = vals[15].segment.selector;
    }
...
#if 1
    {
        ...
        m_vp.RegWrite(regs, vals, ARRAY_SIZE(regs));
    }
#endif
...
int GdbServer::dbg_mem_read(char *buf, size_t buf_len, address addr, size_t len, dbg_enc_func enc)
{
    ...
    for (pos = 0; pos < len; pos++) {
        if (dbg_sys_mem_readb((address)(addr+pos), &data[pos])) {
            /* Failed to read */
            return EOF;
        }
    }
...
int GdbServer::dbg_mem_write(const char *buf, size_t buf_len, address addr, size_t len, dbg_dec_func dec)
{
    ...
    for (pos = 0; pos < len; pos++) {
        if (dbg_sys_mem_writeb((address)(addr+pos), data[pos])) {
            /* Failed to write */
            return EOF;
        }
    }
